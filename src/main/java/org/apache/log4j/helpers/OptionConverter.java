...
public class OptionConverter {
    ...
    public void selectAndConfigure(URL url, String clazz, LoggerRepository hierarchy) {
        ...
        /*
        选择配置文件解析类（用于解析配置文件中的配置信息）：
            若用户配置了属性，就用改属性的值作为配置文件解析类；
            若用户没有配置属性：
                若配置文件以.xml结尾，就用org.apache.log4j.xml.DOMConfigurator作为配置文件解析类
                若配置文件不以.xml结尾，则用org.apache.log4j.PropertyConfigurator作为配置文件解析类
        */
