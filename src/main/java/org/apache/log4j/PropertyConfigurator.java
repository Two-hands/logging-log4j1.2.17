...
public class PropertyConfigurator implements Configurator {
    ...
    public void doConfigure(Properties properties, LoggerRepository hierarchy) {
        ...
        //若设置log4j.debug属性，开启log的debug级别的log日志
        ...
        //若设置log4j.reset=true，在配置剩余属性之前进行hierarchy的重置
        String reset = properties.getProperty(RESET_KEY);
        if (reset != null && OptionConverter.toBoolean(reset, false)) {
            hierarchy.resetConfiguration();
        }

        //若设置log4j.threshold属性，重置hierarchy的threshold属性
        //tips：threshold域值将决定appender对低于该级别的日志事件会被忽略
  ...
  */
    ...
    void configureRootCategory(Properties props, LoggerRepository hierarchy) {
        //尝试获取log4j.rootLogger的属性值
        String effectiveFrefix = ROOT_LOGGER_PREFIX;
        String value = OptionConverter.findAndSubst(ROOT_LOGGER_PREFIX, props);

        if (value == null) {
            //若log4j.rootLogger属性不存在，在尝试获取log4j.rootCategory的属性值
    ...
    protected void parseCatsAndRenderers(Properties props, LoggerRepository hierarchy) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (key.startsWith(CATEGORY_PREFIX) || key.startsWith(LOGGER_PREFIX)) {
                //属性key以log4j.category. 或者 log4j.logger. 开头
                ...
                //属性key以log4j.throwableRenderer开头
    ...
    void parseCategory(Properties props, Logger logger, String optionKey, String loggerName, String value) {
        ...
        // 移除rootLogger中所有的apeender实例，即初始化
